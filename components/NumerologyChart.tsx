...
import type { NumerologyChart as NumerologyChartType, NumerologyResults, NumerologyInterpretation } from "@/lib/types"
import { CATEGORY_MAP, CATEGORY_DESCRIPTIONS } from "@/lib/numerology-constants"
import { Shield, Flame, Heart } from "lucide-react"

interface NumerologyChartProps {
  chart: NumerologyChartType
}

export default function NumerologyChart({ chart }: NumerologyChartProps) {
  const formattedDate = new Date(chart.birth_date).toLocaleDateString("en-GB", {
    day: "numeric",
    month: "long",
    year: "numeric",
  })
  ...
  function findInterpretation(category: string, number: number): NumerologyInterpretation | undefined {
    ...
    return chart.results_snapshot[key] as NumerologyInterpretation | undefined
  }
    ...
    <div className="max-w-reading mx-auto space-y-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {(Object.keys(CATEGORY_MAP) as (keyof NumerologyResults)[]).map((key) => {
          const number = chart.results[key]
